...
KEYPAD_enumError_t KEYPAD_Init()
{
    ...
    for (loc_iterator = 0; loc_iterator < KEYPAD_NUMBER_OF_ROWS; loc_iterator++)
    {
        ...
        loc_PORTstr.Mode = GPIO_MODE_IN_PU;														/* Configure the Required Pin to Input Pull Up */
        GPIO_Init(&loc_PORTstr,1);																        /* Sending the Configuration to the Port To Make the Configurations Take Action */
    }
    ...
    for (loc_iterator = 0; loc_iterator < KEYPAD_NUMBER_OF_COLS; loc_iterator++)
    {
        ...
        loc_PORTstr.Mode = GPIO_MODE_OP_PP;														/* Configure the Required Pin to Output */
        GPIO_Init(&loc_PORTstr,1);																	    /* Sending the Configuration to the Port To Make the Configurations Take Action */
        GPIO_Set_PinValue(KEYPAD_strConfig.KEYPAD_strCOLS[loc_iterator].KEYPAD_enumPORT,KEYPAD_strConfig.KEYPAD_strCOLS[loc_iterator].KEYPAD_enumPin,KEYPAD_strConfig.KEYPAD_enumLogicType);
...
void Keypad_Runnable(void)
{
    ...
    for (loc_iteratorCols = 0; loc_iteratorCols < KEYPAD_NUMBER_OF_COLS; loc_iteratorCols++)
    {
        ...
        GPIO_Set_PinValue(KEYPAD_strConfig.KEYPAD_strCOLS[loc_iteratorCols].KEYPAD_enumPORT,KEYPAD_strConfig.KEYPAD_strCOLS[loc_iteratorCols].KEYPAD_enumPin,GPIO_STATE_RESET);

        for (loc_iteratorRows = 0; loc_iteratorRows < KEYPAD_NUMBER_OF_ROWS; loc_iteratorRows++)
        {
            /* Read the value for each switch in each row and hold it in loc_pinValue */
            GPIO_Get_PinValue(KEYPAD_strConfig.KEYPAD_strROWS[loc_iteratorRows].KEYPAD_enumPORT,KEYPAD_strConfig.KEYPAD_strROWS[loc_iteratorRows].KEYPAD_enumPin,&SW_CurrentState);
        ...
        GPIO_Set_PinValue(KEYPAD_strConfig.KEYPAD_strCOLS[loc_iteratorCols].KEYPAD_enumPORT,KEYPAD_strConfig.KEYPAD_strCOLS[loc_iteratorCols].KEYPAD_enumPin,GPIO_STATE_SET);
    }
}