...
u16 Switch_State[KEYPAD_NUMBER_OF_ROWS][KEYPAD_NUMBER_OF_COLS];
...
KEYPAD_enumError_t KEYPAD_Init()
{
    ...
    {
        loc_PORTstr.GPIO_Port = KEYPAD_strConfig.KEYPAD_strROWS[loc_iterator].KEYPAD_enumPORT;			/* Assign The Keypad Rows Ports to the local Port Struct */
        loc_PORTstr.GPIO_Pin = KEYPAD_strConfig.KEYPAD_strROWS[loc_iterator].KEYPAD_enumPin;			/* Assign The Keypad Cols Pins to the Local Port Struct */
        loc_PORTstr.GPIO_Mode = GPIO_MODE_IN_PU;														/* Configure the Required Pin to Input Pull Up */
    ...
    {
        loc_PORTstr.GPIO_Port = KEYPAD_strConfig.KEYPAD_strCOLS[loc_iterator].KEYPAD_enumPORT;			/* Assign The Keypad Rows Ports to the local Port Struct */
        loc_PORTstr.GPIO_Pin = KEYPAD_strConfig.KEYPAD_strCOLS[loc_iterator].KEYPAD_enumPin;			/* Assign The Keypad Cols Pins to the Local Port Struct */
        loc_PORTstr.GPIO_Mode = GPIO_MODE_OUT_PP;														/* Configure the Required Pin to Output */
        GPIO_Init(&loc_PORTstr);																	    /* Sending the Configuration to the Port To Make the Configurations Take Action */
    }
...
KEYPAD_enumError_t KEYPAD_getPressedKey(char* Add_KEYPAD_enumSwitch)
{
    ...
    {
        loc_checkValue = GPIO_STATE_RESET;
    }
...
void Keypad_Runnable(void)
{
    ...
    u8 loc_iteratorCols = 0;
	u8 SW_CurrentState;
    static u8 prev[KEYPAD_NUMBER_OF_COLS][KEYPAD_NUMBER_OF_ROWS] = {0};
	static u8 counts[KEYPAD_NUMBER_OF_COLS][KEYPAD_NUMBER_OF_ROWS] = {0};

    /* Iterates over the keypad matrix by setting the columns with logic high and then checking each row */
    for (loc_iteratorCols = 0; loc_iteratorCols < KEYPAD_NUMBER_OF_COLS; loc_iteratorCols++)
    {
        /* Set each column of the keypad matrix with logic high to check every row */
        GPIO_setPinValue(KEYPAD_strConfig.KEYPAD_strCOLS[loc_iteratorCols].KEYPAD_enumPORT,KEYPAD_strConfig.KEYPAD_strCOLS[loc_iteratorCols].KEYPAD_enumPin,GPIO_STATE_RESET);
        ...
        GPIO_setPinValue(KEYPAD_strConfig.KEYPAD_strCOLS[loc_iteratorCols].KEYPAD_enumPORT,KEYPAD_strConfig.KEYPAD_strCOLS[loc_iteratorCols].KEYPAD_enumPin,GPIO_STATE_SET);
    }
}