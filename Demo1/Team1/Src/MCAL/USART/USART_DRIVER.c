...
USART_Init(USART_cfg_t USART_CfgArr)
{
    ...
    float32_t LOC_BRRValue = 0;
    uint32_t LOC_CR1Value = 0;
    uint32_t LOC_CR2Value = 0;
    uint16_t LOC_Mantissa;
    uint16_t LOC_Fraction;
    uint32_t LOC_USARTDIV;
    uint32_t LOC_OverSampling;
    ...
    else
    {
        ...
        LOC_USARTDIV = (((uint64_t)USART_CLK * 1000) / (USART_CfgArr.BaudRate * (8 * (2 - LOC_OverSampling))));
...
{
    Error_Status LOC_Status = Status_NOK;
    volatile uint16_t TimeOut = 3000;
...
Error_Status USART_GenerateBreak(uint8_t USART_Peri)
{
    Error_Status LOC_Status = Status_NOK;
...
Error_Status USART_Set_BreakCallBack(uint8_t USART_Peri, CallBack_t CB)
{
    Error_Status LOC_Status = Status_NOK;
